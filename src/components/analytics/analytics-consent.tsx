...
export function AnalyticsConsent() {
  ...
  return (
    <aside
      role="dialog"
      aria-label={t[0]}
      className="fixed inset-x-2 bottom-2 z-50 mx-auto max-w-xl rounded-2xl border border-white/20 bg-[#17231c]/[.98] p-3 text-white shadow-2xl backdrop-blur sm:inset-x-auto sm:bottom-3 sm:left-1/2 sm:w-[min(24rem,calc(100vw-2rem))] sm:p-3.5 sm:-translate-x-1/2"
    >
      <div className="flex flex-col items-start justify-between gap-2 sm:flex-row sm:gap-2.5">
        <div>
          <p className="text-sm font-bold">{t[0]}</p>
          <p className="mt-1 max-w-[19rem] text-[11px] leading-4 text-white/75 sm:max-w-none sm:text-xs">{t[1]}</p>
        </div>
