...
export function HomePage({ locale, products = fixtureProducts }: { locale: Locale; products?: Product[] }) {
  ...
  const collectionProducts = [
    { label: t.home.music, title: localize(hero.tagline, locale), product: hero },
    { label: t.nav.explore, title: localize(findProduct(liveProducts, "gl1").tagline, locale), product: findProduct(liveProducts, "gl1") },
    { label: t.home.capture, title: localize(findProduct(liveProducts, "v03-t5").tagline, locale), product: findProduct(liveProducts, "v03-t5") },
    { label: t.shop.newest, title: localize(findProduct(liveProducts, "gl6").tagline, locale), product: findProduct(liveProducts, "gl6") },
  ];
  ...
  return <>
    <section className="border-b border-[var(--line)] bg-[var(--paper)]">
      <div className="shell grid items-stretch lg:min-h-[540px] lg:grid-cols-[.93fr_1.07fr]">
        <div className="flex flex-col justify-between py-12 md:py-16 lg:py-14">
          <div>
            <p className="eyebrow text-[#7f9412]">{t.home.choose}</p>
            ...
            <div className="mt-9 flex flex-wrap items-center gap-4">
              <Link href={`/${locale}/shop`} className="button-primary px-6 py-3.5">{t.common.viewAll} <ArrowRight size={18} /></Link>
              <Link href={`/${locale}/products/${hero.slug}`} className="group inline-flex items-center gap-2 text-sm font-bold underline decoration-[var(--lime)] decoration-2 underline-offset-4">
                {t.common.learnMore}: {localize(hero.name, locale)} <ArrowUpRight className="transition-transform group-hover:-translate-y-0.5 group-hover:translate-x-0.5" size={16} />
              </Link>
            </div>
          </div>
          <div className="mt-14 grid max-w-xl grid-cols-3 border-y border-[var(--line)] py-5 text-xs leading-5 text-[var(--muted)]">
            <span><b className="block text-[var(--ink)]">{liveProducts.length}</b>{t.footer.product}</span>
            <span><b className="block text-[var(--ink)]">USD</b>{t.common.finalUsd}</span>
            <span><b className="block text-[var(--ink)]">24/7</b>{t.nav.support}</span>
          </div>
        </div>
        <Link href={`/${locale}/products/${hero.slug}`} className="group relative flex min-h-[480px] items-center justify-center overflow-hidden bg-white px-5 py-12 md:min-h-[620px] lg:min-h-full lg:px-14">
          <span className="absolute start-6 top-6 z-10 text-[10px] font-bold uppercase tracking-[.18em] text-[var(--muted)]">{t.shop.newest} / {localize(hero.name, locale)}</span>
    ...
    <section className="shell py-16 md:py-24">
      <div className="flex flex-wrap items-end justify-between gap-6 border-b border-[var(--ink)] pb-5">
        <div><p className="eyebrow">{t.home.choose}</p><h2 className="mt-4 text-4xl font-medium tracking-[-.07em] md:text-5xl">{t.shop.title}</h2></div>
        <Link href={`/${locale}/shop`} className="group inline-flex items-center gap-2 text-sm font-bold">{t.common.viewAll} <ArrowRight className="transition-transform group-hover:translate-x-1 rtl:rotate-180" size={17} /></Link>
      </div>
    ...
    <section className="border-y border-[var(--line)] bg-white">
      <div className="shell py-16 md:py-24">
        <div className="grid gap-8 lg:grid-cols-[.82fr_1.18fr] lg:items-end">
          <div><p className="eyebrow text-[#7f9412]">{t.shop.title}</p><h2 className="mt-4 max-w-md text-4xl font-medium tracking-[-.07em] md:text-5xl">{t.home.scenarios}</h2><p className="mt-5 max-w-sm leading-7 text-[var(--muted)]">{t.shop.intro}</p></div>
    ...
    <section className="shell py-16 md:py-24"><div className="grid border-y border-[var(--line)] md:grid-cols-3"><Value title={t.product.specs} number="01" text={t.product.dispatch} /><Value title={t.common.chooseColor} number="02" text={t.product.lensLink} /><Value title={t.checkout.review} number="03" text={t.checkout.paymentCopy} /></div></section>
...
function EditorialProduct({ product, locale }: { product: Product; locale: Locale }) {
  ...
  return <article className="group min-w-0 py-6 sm:px-5 sm:py-7 first:sm:ps-0 last:sm:pe-0 xl:px-6"><Link href={`/${locale}/products/${product.slug}`} className="block"><div className="relative flex aspect-[1.15/1] items-center justify-center overflow-hidden bg-[var(--paper)] p-5"><Image src={product.heroImage} alt={name} width={640} height={560} className="h-full w-full object-contain transition duration-500 group-hover:scale-[1.045]" sizes="(max-width: 640px) 100vw, (max-width: 1280px) 50vw, 25vw" /><span className="absolute end-4 top-4 grid h-8 w-8 place-items-center rounded-full bg-[var(--lime)] opacity-0 transition group-hover:opacity-100"><ArrowUpRight size={15} /></span></div><p className="mt-5 text-[10px] font-bold uppercase tracking-[.16em] text-[var(--muted)]">{t.common.demo}{colorCount > 1 ? ` / ${colorCount}` : ""}</p><h3 className="mt-2 text-2xl font-medium leading-[.94] tracking-[-.055em]">{name}</h3><p className="mt-3 min-h-10 text-sm leading-5 text-[var(--muted)]">{localize(product.tagline, locale)}</p><p className="mt-5 text-lg font-bold">{formatUsd(product.usdPrice)} {product.compareAtUsdPrice && <del className="ms-2 text-sm font-medium text-[var(--muted)]">{formatUsd(product.compareAtUsdPrice)}</del>}</p><span className="mt-5 inline-flex items-center gap-2 text-sm font-bold underline decoration-[var(--lime)] decoration-2 underline-offset-4">{t.common.learnMore} <ArrowRight className="rtl:rotate-180" size={15} /></span></Link></article>;
}
