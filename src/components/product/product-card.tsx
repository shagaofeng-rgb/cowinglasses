  ...
  return (
    <article className="group border-t border-[var(--line)] pt-3">
      ...
      <div className="flex items-start justify-between gap-3 pt-4">
        <div>
          <p className="text-[10px] font-bold uppercase tracking-wider text-[var(--muted)]">{product.demo ? t.demo : "Available now"}</p>
          <h2 className="mt-1 text-lg font-bold">{name}</h2>
          <p className="mt-1 max-w-[28ch] text-sm leading-5 text-[var(--muted)]">{localize(product.tagline, locale)}</p>
        </div>
        <Link className="mt-5 grid h-10 w-10 shrink-0 place-items-center rounded-full bg-[var(--lime)] text-black" href={`/${locale}/products/${product.slug}`} aria-label={`View ${name}`}><ArrowUpRight size={17} /></Link>
      </div>
      <div className="pt-4">
