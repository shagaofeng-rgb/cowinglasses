...
async function bootstrap() {
  if (!process.env.DATABASE_URL) {
    console.info("未配置 DATABASE_URL，跳过后台数据库初始化。");
    return;
  }
  ...
  try {
    ...
    const superAdminRole = (await db.select().from(roles)).find((role) => role.code === "super_admin");
...
bootstrap().catch((error) => {
  console.error("后台数据库初始化失败", error);
  process.exitCode = 1;
});
